...
import { RowsPlusBottomIcon, TrashIcon } from "@phosphor-icons/react/dist/ssr";
...
const InstructorCourse: React.FC = () => {
  ...
  const [activePage, setActivePage] = useState<number>(1); // 1-based UI
  ...
  // Query: courses list (pass page+size as args so RTK Query refetches)
  const {
    data: coursesRes,
    isLoading,
    isError,
  ...
  const selectedCourse = useMemo(
    () => courseList.find((c) => c.courseId === selectedCourseId) ?? null,
    [courseList, selectedCourseId]
  );

  const { data: sectionsRes } = useGetCourseListQuery(selectedCourseId as number, {
    skip: !selectedCourseId,
  });

  // Reset students sub-page whenever course changes
  useEffect(() => {
    setSubActivePage(1);
  }, [selectedCourseId]);
   ...
   * Handlers
   *********************************/
  const openDetails = (courseId: number) => {
  ...
  const onDeleteCourse = async (courseId: number) => {
    ...
    try {
      ...
      const msg =
        err?.data?.message || err?.error || "Xóa khóa học thất bại. Vui lòng thử lại.";
  ...
  return (
    <div>
      <p>
        Total courses: <span className="font-bold">{totalItems} courses</span>
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6 mt-5">
        {courseList.map((course) => (
          <Card
            key={course.courseId}
            className="pt-0 pb-6 overflow-clip hover:bg-accent/10 transition-colors duration-50 ease-in-out"
          >
            <div className="flex flex-col flex-1 gap-2">
              <CardHeader
                onClick={() => navigate(`/dashboard/instructor/courses/${course.courseId}/review`)}
            ...
            <DialogContentText id="course-dialog-desc" component="div">
              <Container>
                <div className="grid grid-cols-1 md:grid-cols-4 h-full gap-4 leading-10">
                  ...
                  <div className="md:col-span-3">
                    {(sectionsRes as SectionsResponse | undefined)?.data?.sections?.map(
                      (section) => (
                        <div
                          key={section.sectionId}
                          className="grid grid-cols-1 md:grid-cols-2 text-center"
                        >
                          <Typography variant="body1" className="font-bold">
      ...
      {selectedCourseId && (
        <Dialog
          ...
          fullWidth
        >
          <DialogTitle id="add-session-dialog-title">Add New Section</DialogTitle>
          <DialogContent>
            <AddSession
              name=""
              index={0}
              ...
              setFunc={() => {
                refetch(); // refresh list after adding a session
                closeAddSession();
              }}
            />
          </DialogContent>
