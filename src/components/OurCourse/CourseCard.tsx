...
const CourseCard = ({ courseId, title, category, amountReview, oldPrice, newPrice, score, thumbnail, instructorName, instructorImage }) => {
  ...
  return (
    <Card className='pt-0 pb-6 overflow-clip'>
      <CardHeader className='px-0'>
        <img src={thumbnail} className='w-full h-52 object-cover rounded-t-md' />
        <div className='px-6 mt-3 w-full overflow-hidden'>
          <p className='flex gap-4 justify-between items-start text-xl font-semibold text-ellipsis'>
            <Tooltip title={title}>
      ...
      <CardFooter>
        <div className='flex justify-between gap-2 items-center'>
          <del className='font-semibold text-gray-600 text-opacity-70'>{oldPrice.toLocaleString('en-US', { style: 'currency', currency: 'VND' })}</del>
